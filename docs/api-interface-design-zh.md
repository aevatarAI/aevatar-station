...
| 参数名 | 类型 | 必须 | 说明 |
|--------|------|------|------|
| agentId | Guid? | 否 | 指定代理ID，不提供则自动生成 |
| agentType | string | 是 | 代理类型，从代理类型列表中选择 |
| name | string | 是 | 代理名称 |
| properties | Dictionary<string, object>? | 否 | 代理配置参数，根据代理类型要求提供 |
...
| 参数名 | 类型 | 必须 | 说明 |
|--------|------|------|------|
| guid | Guid | 是 | 父代理唯一标识符 |
| subAgentId | Guid | 是 | 子代理唯一标识符 |
...
|--------|------|------|------|
| guid | Guid | 是 | 父代理唯一标识符 |
| subAgentId | Guid | 是 | 要移除的子代理唯一标识符 |
...
|--------|------|------|------|
| guid | Guid | 是 | 代理唯一标识符 |
| name | string | 是 | 新的代理名称 |
| properties | Dictionary<string, object>? | 否 | 更新的配置参数 |
...
| payload | object | 是 | 事件载荷数据 |
| targetAgentId | Guid? | 否 | 目标代理ID，不指定则广播 |
...
| 参数名 | 类型 | 必须 | 说明 |
|--------|------|------|------|
| projectId | Guid? | 否 | 项目ID，筛选指定项目的插件 |
| pageIndex | int | 否 | 页码，从0开始 |
| pageSize | int | 否 | 每页大小 |
...
| 参数名 | 类型 | 必须 | 说明 |
|--------|------|------|------|
| projectId | Guid | 是 | 目标项目ID |
| code | IFormFile | 是 | 插件代码文件，支持.zip, .dll等格式 |
...
| 参数名 | 类型 | 必须 | 说明 |
|--------|------|------|------|
| type | NotificationTypeEnum | 是 | 通知类型 (Invite=1, Reminder=2, Alert=3) |
...
|--------|------|------|------|
| id | Guid | 是 | 通知ID |
| status | int | 是 | 响应状态 (1=接受, 2=拒绝, 3=忽略) |
...
| 参数名 | 类型 | 必须 | 说明 |
|--------|------|------|------|
| notificationIds | Guid[] | 是 | 要标记已读的通知ID列表 |
...
| code | IFormFileCollection | 是 | 代码文件集合 |
...
| 参数名 | 类型 | 必须 | 说明 |
|--------|------|------|------|
| code | IFormFileCollection | 是 | 新的代码文件集合 |
...
| 参数名 | 类型 | 必须 | 说明 |
|--------|------|------|------|
| webhookId | string | 是 | 要销毁的Webhook ID |
| version | string | 是 | 要销毁的版本号 |
...
| 参数名 | 类型 | 必须 | 说明 |
|--------|------|------|------|
| eventType | string | 是 | 要订阅的事件类型 |
| subscriberId | Guid | 是 | 订阅者ID |
| webhookUrl | string? | 否 | 事件发生时的回调URL |
| filterConditions | object? | 否 | 事件过滤条件 |
...
| startDate | DateTime? | 否 | 统计开始时间 |
| endDate | DateTime? | 否 | 统计结束时间 |
