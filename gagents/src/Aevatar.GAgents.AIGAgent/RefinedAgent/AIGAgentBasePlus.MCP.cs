    ...
    where TConfiguration : ConfigurationBase
{
    ...
    public virtual async Task<bool> ConfigureMCPServersAsync(List<IMCPGAgent> mcpGAgents)
    {
        try
        {
            var mcpAgents = new Dictionary<string, MCPGAgentReferencePlus>();
            ...
            foreach (var mcpAgent in mcpGAgents)
            {
                ...
                mcpAgents[server.ServerName] = new MCPGAgentReferencePlus
                {
                    AgentId = mcpAgentId,
    ...
    {
        [Id(0)] public Dictionary<string, MCPGAgentReferencePlus> MCPServers { get; set; } = new();
    }
