...
public sealed class LLMConfigurationMigrationTest : AevatarAIGAgentBaseTestBase
{
    [Fact]
    public async Task OnGAgentActivateAsync_Should_MigrateSystemLLMToLLMConfigKey_When_LegacyFormatDetected()
    {
        ...
        var agent = await GAgentFactory.GetGAgentAsync<ITestChatAIGAgent>(Guid.NewGuid());
    ...
    public async Task OnGAgentActivateAsync_Should_AutoMigrateOnStartup_When_ExistingAgentHasOnlySystemLLM()
    {
        ...
        var agent = await GAgentFactory.GetGAgentAsync<ITestChatAIGAgent>(agentId);
