...
public class GAgentAsyncObserverPlus : IAsyncObserver<EventWrapperBase>
{
    private readonly List<EventWrapperBaseAsyncObserver> _observers;
    private readonly string _grainId;
    private readonly ILogger<GAgentAsyncObserverPlus> _logger;

    public GAgentAsyncObserverPlus(List<EventWrapperBaseAsyncObserver> observers, string grainId, ILogger<GAgentAsyncObserverPlus> logger)
    {
    ...
    public GAgentAsyncObserverPlus(List<EventWrapperBaseAsyncObserver> observers, string grainId)
    ...
    private List<EventWrapperBaseAsyncObserver> FindMatchingObservers(EventBase eventType)
    {
        return _observers.Where(observer =>
            observer.ParameterTypeName == eventType.GetType().Name).ToList();
    }
