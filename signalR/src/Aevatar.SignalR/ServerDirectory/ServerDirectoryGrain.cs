...
{
    private const int SERVERDIRECTORY_CLEANUP_IN_MINUTES = SignalROrleansConstants.ServerHeartbeatPulseInMinutes * 3;
    ...
    public ServerDirectoryGrain(
        ...
        [PersistentState(nameof(ServerDirectoryState), SignalROrleansConstants.SignalrOrleansStorageProvider)] IPersistentState<ServerDirectoryState> state)
    {
    ...
    public Task OnActivateAsync(CancellationToken cancellationToken)
    {
        ...
        _timerRegistry.RegisterGrainTimer(
            ...
            {
                DueTime = TimeSpan.FromMinutes(SERVERDIRECTORY_CLEANUP_IN_MINUTES),
                Period = TimeSpan.FromMinutes(SERVERDIRECTORY_CLEANUP_IN_MINUTES),
            });
    ...
    private async Task ValidateAndCleanUp(ServerDirectoryState serverDirectory, CancellationToken cancellationToken)
    {
