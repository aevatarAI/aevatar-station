...
/// </summary>
public sealed class AIGAgentBaseMCPTest : AevatarAIGAgentBaseTestBase
{
    ...
    {
        // Arrange
        var agent = await GAgentFactory.GetGAgentAsync<ITestMCPAIGAgent>();
    ...
    {
        // Arrange
        var agent = await GAgentFactory.GetGAgentAsync<ITestMCPAIGAgent>(Guid.NewGuid());
        await agent.InitializeAsync(new InitializeDto
        {
            Instructions = "Test MCP agent",
            LLMConfig = new LLMConfigDto { SystemLLM = "OpenAI" }
