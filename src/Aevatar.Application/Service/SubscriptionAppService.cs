...
public class SubscriptionAppService : ApplicationService, ISubscriptionAppService
{
    ...
    {
        var agent = _clusterClient.GetGrain<ITestAgentWithConfiguration>(dto.AgentId);


        await agent.PublishNumberEventAsync(new SetNumberSEvent()
        {
            Number = int.Parse(dto.EventProperties["Value"].ToString())
        });
        _logger.LogInformation("PublishEventAsync id: {id} ", dto.AgentId);
