...
### F021: Aspire Single Container Dockerfile 🚧
- **分支**: `feature/aspire-single-container-dockerfile`
- **状态**: 🚧 开发中
- **开发机器**: c6:c4:e5:e8:c6:4b
- **描述**: 创建包含所有Aevatar.Aspire服务的单容器Dockerfile，替代docker-compose多容器架构
- **进度**: 
  - ✅ 创建包含所有服务的单容器Dockerfile (使用.NET 9 SDK)
  - ✅ 配置MongoDB、Redis、Elasticsearch、Kafka、Zookeeper等基础服务
  - ✅ 配置监控服务：Prometheus、Grafana、Jaeger、OpenTelemetry
  - ✅ 创建服务启动脚本处理服务依赖和顺序启动
  - ✅ 创建构建和测试脚本
  - ✅ 采用逐行安装方式重构Dockerfile，参考Microsoft SDK最佳实践
  - 🚧 测试验证所有服务在单容器中正常运行
- **覆盖率**: 单容器包含12个服务：MongoDB(2实例)、Redis、Elasticsearch、Kafka、Zookeeper、Qdrant、Jaeger、OpenTelemetry、Prometheus、Grafana、Blackbox Exporter
