...
| Auto-Generated Domain Names for New Projects | ✅ | feature/auto-generated-domain-names | c6:c4:e5:e8:c6:4b | ✅已完成：实现项目创建的自动域名生成功能，完成API简化重构。统一使用`CreateProjectAsync`方法和`POST /api/app/project`接口，移除原有`CreateAsync`方法。基于项目名称自动生成域名，冲突时抛出UserFriendlyException保持一致性。代码架构完全优化：遵循DRY原则，提取共享私有方法CreateProjectInternalAsync，显式依赖注入ILogger，利用框架[Required]验证移除冗余参数校验。单元测试完全整合到ProjectServiceTests.cs，删除冗余测试文件。完整技术方案文档（中英文版本）均已完成。 |