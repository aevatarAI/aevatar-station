...
| WorkflowRunRecordGAgent 日志记录优化 | ✅ | feature/workflow-execution-record-logging-optimization | 42:82:57:47:65:d4 | ✅已完成：创建全新WorkflowRunRecordGAgent代理替换原WorkflowExecutionRecordGAgent。采用纯"点"记录模式专注单个Agent节点执行，通过BeforeAgentIds追踪数据血缘，使用JSON字符串存储避免重复，单一CurrentStateJson实时状态根据执行阶段动态更新。移除EventFlowRecord简化设计，专注核心工作流执行追踪。完整编译通过。 |